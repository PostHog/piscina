...
class ThreadPool {
    ...
    runTask(task, transferList, filename, abortSignal, workerInfo) {
        if (filename === null) {
            filename = this.options.filename;
        }
...
class Piscina extends eventemitter_asyncresource_1.default {
    ...
    runTask(task, transferList, filename, abortSignal, workerInfo) {
        ...
        return __classPrivateFieldGet(this, _Piscina_pool, "f").runTask(task, transferList, filename || null, abortSignal || null, workerInfo || null);
    }
    broadcastTask(task, transferList, filename, abortSignal) {
        const promises = [];
